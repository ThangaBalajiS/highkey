export class VisualSelector {
  ...
  private generateResilientSelector(el: HTMLElement): string {
    ...
    // 2. Stable Data Attributes or Aria
    const stableAttrs = ['data-testid', 'data-cy', 'aria-label', 'name', 'placeholder'];
    ...
    // 4. Short Ancestor Anchor
    let parent = el.parentElement;
    while (parent && parent.tagName !== 'BODY') {
      if (parent.id && !this.isAutoGenerated(parent.id)) {
        return `#${CSS.escape(parent.id)} ${tag}`;
      }
      const pClasses = Array.from(parent.classList).filter(c => !this.isTailwindOrHashed(c));
      if (pClasses.length > 0) {
        return `${parent.tagName.toLowerCase()}.${CSS.escape(pClasses[0])} ${tag}`;
      }
