export class ShortcutEngine {
  ...
  private async findWithRetry(selector: string, retries = 5, interval = 100): Promise<HTMLElement | null> {
  ...
  private deepQuerySelector(selector: string, root: Document | ShadowRoot = document): HTMLElement | null {
  ...
  private simulateFullClick(el: HTMLElement) {
    ...
    if (window.PointerEvent) {
      const pointerOptions = { ...eventOptions, pointerId: 1, pointerType: 'mouse', isPrimary: true, buttons: 0 };
      deepTarget.dispatchEvent(new PointerEvent('pointerup', pointerOptions));
    }
    
    const mouseUpOptions = { ...eventOptions, buttons: 0 };
