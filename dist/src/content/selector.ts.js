export class VisualSelector {
	...
	async saveShortcut(shortcut, selector, scope) {
		...
		const label = this.lastElement?.innerText?.trim().slice(0, 50) || this.lastElement?.getAttribute("aria-label") || this.lastElement?.getAttribute("title") || "Unnamed Shortcut";
		const data = await chrome.storage.local.get("shortcuts") || {};
		const shortcuts = data.shortcuts || {};
		if (!shortcuts[domain]) shortcuts[domain] = [];
		if (this.remappingId) {
			shortcuts[domain] = shortcuts[domain].map((s) => s.id === this.remappingId ? {
				...s,
				selector,
				label,
				scope,
